#!/usr/bin/env python3

import numpy as np
import h5py
import math
from matplotlib import pyplot as plt

# Simulation parameters
clight = 299792458.0
l0 = 1e-6
dx = 0.05*l0
cflFactor = 0.99
dt = cflFactor*dx/clight
omega = 2*math.pi*clight/l0
# todo, we can make the analysis better by using the numerical dispersion relation


time = np.arange(0, 61, 1)
Bx_err = 0.0*time
By_err = 0.0*time
Bz_err = 0.0*time
Ex_err = 0.0*time
Ey_err = 0.0*time
Ez_err = 0.0*time


for tstep in range(61):
  print(tstep)
  fBx = h5py.File('Bx_{}.h5'.format(tstep), 'r')
  Bx = fBx['data'][:,:]
  fBy = h5py.File('By_{}.h5'.format(tstep), 'r')
  By = fBy['data'][:,:]
  fBz = h5py.File('Bz_{}.h5'.format(tstep), 'r')
  Bz = fBz['data'][:,:]
  fEx = h5py.File('Ex_{}.h5'.format(tstep), 'r')
  Ex = fEx['data'][:,:]
  fEy = h5py.File('Ey_{}.h5'.format(tstep), 'r')
  Ey = fEy['data'][:,:]
  fEz = h5py.File('Ez_{}.h5'.format(tstep), 'r')
  Ez = fEz['data'][:,:]

  L = Bz.size
  Lx = L*dx

  xs = np.arange(start=0.5*dx, stop=Lx, step=dx)
  x = np.arange(start=0.0*dx, stop=Lx, step=dx)
  t = tstep*dt
  ts = (tstep + 0.5)*dt

  Bx_err[tstep] = clight**2 * np.square(Bx).max()
  By_err[tstep] = clight**2 * np.square(By).max()
  Bz_err[tstep] = clight**2 * np.square(Bz).max()
  Ex_err[tstep] = np.square(Ex).max()
  Ey_err[tstep] = np.square(Ey).max()
  Ez_err[tstep] = np.square(Ez).max()
  # if tstep % 5 == 0:
  #   plt.plot(Ey, 'k-', linewidth = 2)
  #   plt.show()

plt.plot(time, Bx_err, 'k-', linewidth = 2)
plt.plot(time, By_err, 'g-', linewidth = 2)
plt.plot(time, Bz_err, 'y-', linewidth = 2)
plt.plot(time, Ex_err, 'b-', linewidth = 2)
plt.plot(time, Ey_err, 'r-', linewidth = 2)
plt.yscale('log')
plt.show()
